...
// MARK : Main functions.
var _convert = function _convert(inputs) {

  // Validate our inputs
  var validate = new Promise(function (resolve, reject) {
    _validateArguments(inputs, function (options) {
      if (options) {
        resolve(options);
      } else {
  ...
  // Validate paths
  validate.then(function (options) {

    var process = new Promise(function (resolve, reject) {
      // Process

      _validatePaths(options, function (status) {
        if (status) {
          resolve(options);
        } else {
    ...
    process.then(function (options) {
      // Process our data and save it
      _processData(options, function (status) {
        console.log(status.info);
      });
    }, function (error) {
      var err = "Something went wrong during processing : " + error;
      console.log(err.error);
    });
  }, function (error) {
    var err = "Something went wrong during validation : " + error;
    console.log(err.error);
  });
};

// MARK Exported methods
var convert = function convert(path, dest, format, tableId) {
  // call internal convert
  var inputs = { "mode": "file", "path": path, "dest": dest, "format": format, "tableId": tableId };
  _convert(inputs);
};

var convertUrl = function convertUrl(url, dest, format, tableId) {
  // call internal convert
  var inputs = { "mode": "url", "url": url, "dest": dest, "format": format, "tableId": tableId };
  _convert(inputs);
};
...
function _writeData(tablesAsJson, tableIds, options) {
  ...
  //File to write
  tablesAsJson.forEach(function (table, index, arr) {
    ...
    if (options.format == 'csv') {
      ...
      fs.writeFile(filePath, JSON.stringify(table), function (err) {
        if (err) throw err;
